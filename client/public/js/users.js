...
function loginUser() {
  ...
  .then((token) => {
    if(token.result != false) {
      ...
      alert("Your username or password is incorrect. User login fail.");
    }
  })
  .catch((error) => {
...
function displayProfile() {
    ...
    htmlProfile =
    '<div class="row"><div class="col-md-8 mt-4 mx-auto text-center"><img src="http://127.0.0.1:8080/uploads/' +
    user_info_array.userPhoto +
    '" width="120px" height="120px" class="rounded-circle border border-secondary"><br>\
    <h5 class="mt-3 text-center" colspan="2">'+ user_info_array.userName +'\'s personal information</h5>';
    htmlProfile += '<div class="mx-auto" style="width: 70%"><table class="table text-left">\
    <tbody><tr><td>Gender:</td><td>'+ user_info_array.userGender +'</td></tr> \
    <tr><td>Contact number:</td><td>'+ user_info_array.userContact +'</td></tr> \
    <tr><td>Email address:</td><td>' + user_info_array.userEmail +'</td></tr>';
    if(user_info_array.userBio) {
        htmlProfile += '<tr><td>Bio:</td><td>'+ user_info_array.userBio +'</td></tr></tbody></table></div>';
    }
...
function deleteUser() {
  var request = new XMLHttpRequest();
  console.log(localStorage.getItem("login_userId"));
  request.open("DELETE", "http://127.0.0.1:8080/users/" + localStorage.getItem("login_userId"), false);
  //This function will be called when data returns from the web api
  request.onload = function () {
    alert("This account is deleted successfully.");
    localStorage.removeItem("token");
    localStorage.removeItem("user_info_string");
    localStorage.removeItem("login_userId");
    window.location.href = "../index.html";
